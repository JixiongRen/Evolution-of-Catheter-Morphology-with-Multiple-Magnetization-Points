...
class GravityRigid:
    ...
    def tree_flatten(self):
        children = (jnp.asarray(self.g_world, dtype=jnp.float64), jnp.asarray(self.r_cm_body, dtype=jnp.float64))
        aux = float(self.mass)
        return children, aux

    @classmethod
    def tree_unflatten(cls, aux, children):
        g_world, r_cm_body = children
        return cls(mass=float(aux), g_world=g_world, r_cm_body=r_cm_body)
