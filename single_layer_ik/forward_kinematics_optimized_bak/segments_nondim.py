...
class FlexibleParams:
    """Parameters for one flexible Cosserat segment.

    This class is registered as a JAX PyTree so that higher-level parameter
    containers (e.g., ``SolverParams``) can be passed through ``jax.jit``
    without being treated as *static* Python objects.

    ``length`` is kept as auxiliary (static) data; the remaining fields are
    JAX arrays.
    """
    ...
    def tree_flatten(self):
        children = (self.Kse_inv, self.Kbt_inv, self.v_star, self.u_star)
        aux = float(self.length)
        return children, aux

    @classmethod
    def tree_unflatten(cls, aux, children):
        Kse_inv, Kbt_inv, v_star, u_star = children
        return cls(length=float(aux), Kse_inv=Kse_inv, Kbt_inv=Kbt_inv, v_star=v_star, u_star=u_star)

@jax.tree_util.register_pytree_node_class
@dataclass(frozen=True)
class RigidParams:
    """Parameters for one rigid segment.

    Registered as a JAX PyTree for the same reason as :class:`FlexibleParams`.
    """
    ...
    def tree_unflatten(cls, aux, children):
        (v_star,) = children
        return cls(length=float(aux), v_star=v_star)
