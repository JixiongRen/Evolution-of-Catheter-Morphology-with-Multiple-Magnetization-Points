...
def compute_scales_from_flex(
    *,
    ...
    # robust central tendency (avoid one axis dominating scale)
    F_int_ref = float(jnp.median(diag_Kse))
    # moment scale corresponds to curvature ~ 1/L_ref: m ~ K_bt * (1/L_ref)
    ...
    if enable_magnetics:
        ...
        if has_inputs and has_pose and has_geom: # and os.path.isfile(str(calib_file)):
            try:
                ...
                # nominal magnet positions: center of each rigid segment (after its flexible segment)
                cum = 0.0
                f_sum = 0.0
                tau_sum = 0.0
                for i, m_body in enumerate(m_body_list):
                    ...
                    # midpoint of rigid segment i along the nominal axis
                    s_mid = cum + lf + 0.5 * lr
                ...
                # Use sum of magnitudes as a conservative "typical" scale.
                F_ext_typ = float(f_sum)
                M_ext_typ = float(tau_sum)
            except Exception:
                # Keep external terms at 0 if calibration/model errors occur.
                F_ext_typ = 0.0
                M_ext_typ = 0.0
...
def build_solver_params(
    *,
    ...
    if coil_currents is None:
        coil_currents_out = jnp.zeros((8,), dtype=jnp.float64)
    else:
...
def main():
    ...
    # --- nondim diagnostics (one-time) ---
    print(
        f"[NONDIM] L_ref={scales.L_ref:.6g}  F_ref={scales.F_ref:.6g}  M_ref={scales.M_ref:.6g}  "
        f"(enable_magnetics={enable_magnetics})"
    )
